...
mkdir -p "${LOG_DIR}"
cd "${ROOT}/code"
ARGS=(python -u "${ROOT}/code/train_sliceeq_occ_ablation.py" --root_path "${DATA_ROOT}" --pretrained_checkpoint "${PRETRAINED_CHECKPOINT}" --exp "${EXP_NAME}" --occ_ablation "${OCC_ABLATION}" --max_iterations 30000 --batch_size 24 --labeled_bs 12 --labelnum 7 --seed 1337)
if [[ "${DETACH}" == "1" ]]; then
